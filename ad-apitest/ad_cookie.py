...
# 查表记录
def listdb():
    db = pymysql.connect(host="192.168.1.183", port=3306, user="root", passwd="123456", db="test")
    cursor = db.cursor()
...
def insertdb():
    ...
    insertsql = "insert into ad_cookie (loginUser,token,updatetime,evn) values ('%s','%s','%s','%s')" % (
        username, cookiestr, updatetime, 'devmarket')
...
def updatedb():
    db = pymysql.connect(host="192.168.1.183", port=3306, user="root", passwd="123456", db="test")
    cursor = db.cursor()
    updatesql = "update ad_cookie set token='%s',updatetime='%s' where loginUser='%s' and evn='devmarket'" % (
